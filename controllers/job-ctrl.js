...
uploadJob = async (req, res) => {
  ...
  job
    ...
    .then(() => {
      ...
      // runPhase(process.env.CSV_FILES + "/" + job._id + "/" + 'train.csv', job._id, job.durationLimit, job.targetColumnName, '***REMOVED***', job.name, true)
      return res.status(201).json({
        success: true,
        id: job._id,
        message: "Job created!",
  ...
  Job.findOne({ _id: req.params.id }, (err, job) => {
    ...
    job
      ...
      .then(() => {
        ...
        // runPhase(process.env.CSV_FILES + "/" + job._id + "/" + id + '.csv', job._id, job.durationLimit, job.targetColumnName, '***REMOVED***', job.name, false)
